...
class AlmacenController {
  ...
  async listarAlmacen(req, res) {
    try {
      const almacen = await Almacen.find().populate('articulos.articulo');
  ...
  // GET /api/almacen/:id - Obtener uno
  async obtenerAlmacen(req, res) {
    try {
      const almacen = await Almacen.findById(req.params.id).populate('productos.producto');
      if (!almacen) {
      ...
      console.error(error);
      res.status(500).json({ status: 'error', message: 'Error al obtener Almacen' });
    }
  }
  ...
  // PATCH /api/almacen/:id - Actualizar
  async actualizarAlmacen(req, res) {
    try {
      const almacen = await Almacen.findByIdAndUpdate(
        req.params.id,
        req.body,
  ...
  // DELETE /api/almacen/:id - Eliminar
  async eliminarAlmacen(req, res) {
    try {
      ...
      console.error(error);
      res.status(500).json({ status: 'error', message: 'Error al eliminar Almacen' });
    }
  }
}
