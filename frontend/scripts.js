...
window.handleEditCategoria = async (id) => {
  const c = CATEGORIAS.find(x => x._id === id);
  if (!c) return toast('Categoría no encontrada', '#ea5455');

  openModal('Editar artículo', [
    { id: 'nombre', label: 'Nombre', type: 'text', value: c.nombre, required: true },
    { id: 'imagen', label: 'URL imagen', type: 'text', value: c.imagen },
  ], async () => {
    const body = {
      nombre: document.getElementById('nombre').value,
      imagen: document.getElementById('imagen').value,
    ...
    toast('Categoría actualizada', '#ff9f43');
...
// 2. Establecer los atributos de la imagen
imagen1.src = './assets/images/FondoYOUTUBE_copia.jpg';
const imagen2 = 'https://cdn.pixabay.com/photo/2016/03/27/22/16/fashion-1284496_1280.jpg';
imagen1.alt = 'Zapatos';

// 3. Encontrar un elemento padre existente en el HTML
// Reemplaza 'idDelElemento' con el ID real de un elemento en tu HTML
const elementoPadre = document.getElementById('tbody-categorias');

// 4. Añadir la imagen al elemento padre
// elementoPadre.appendChild(imagen1);
...
function renderCategorias() {
  ...
  tbodyCat.innerHTML = filtered.map(c => `
    <tr>
      ...
      <td>${`<img src=${imagen2}></img>`}</td>
      
     <td>
        <button class="btn edit" onclick='handleEditCategoria("${c._id}")'>✏️</button>
